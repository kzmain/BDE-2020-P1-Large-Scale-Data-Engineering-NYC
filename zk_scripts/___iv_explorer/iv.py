...
def get_nearest(return_colors=False):
...
def main():
    ...
    data = filter_columns(gpd.read_file("a.json"), keep_columns)

    # Rename columns
    data.rename(
        columns={'he_vakiy': 'pop2018', 'pinta_ala': 'area', 'nimi': 'name', 'hr_mtu': 'income', 'posti_alue': 'zip'},
        inplace=True)
